...
export enum CountriesV2ApiApiKeys {
}

export class CountriesV2Api {
    ...
    public setApiKey(key: CountriesV2ApiApiKeys, value: string) {
        (this.authentications as any)[CountriesV2ApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getCountries (offset?: number, limit?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetCountries200Response;  }> {
        const localVarPath = this.basePath + '/v2/countries';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetCountries200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetCountries200Response");
